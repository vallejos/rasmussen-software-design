# filename displayWallResults.py

# The displayWallResults takes "area", "gallons" and "size" as parameters
# and displays the total room area to paint for each room as well as how
# many gallons are required for that painting. Finally, it displays the totals.

def displayWallResults(area, gallons, size):
  index = int(0)
  totalArea = float(0)
  totalGallons = float(0)
  
  for index in range(0, size):
    print "Total for Wall %d" % (index + 1) 
  ...
  print "Total wall's square feet to paint: %4.2f" % totalArea
